...
namespace SampleDB
{
    public partial class EditCommunicationsPage : ContentPage
    {

        public List<Category> listOfCategarys;
        public List<Ratings> listOfRatings;
        public List<EntBehavior> listOfEnterprisePerCategary;
        public List<OrgBehavior> listOfBehaviersPerCategary;
        ...
        public List<Category> arrayobj;

        public EditCommunicationsPage(int Org_Id, string jsseid)
        {
            InitializeComponent();
        ...
        protected async override void OnAppearing()
        {
           ...
           // listOfCategarys = arrayobj;

            DynamicCategaryDesign(arrayobj);

        }


        public async void DynamicCategaryDesign(List<Category> listOfCategarys)
        {


            for (int i = 0; i < listOfCategarys.Count; i++)
            {
                listOfRatings = listOfCategarys[i].Ratings;
                ...
                titleLabel.Text = listOfCategarys[i].CategoryDesc;
                ...
                CategaryStacklayout.StyleId = listOfCategarys[i].CategoryDesc.ToString();
                ...
                listOfEnterprisePerCategary = listOfCategarys[i].EntBehaviors;
                listOfBehaviersPerCategary = listOfCategarys[i].OrgBehaviors;

                VisibleOrInVisibleScreenDesign(listOfRatings, CategaryStacklayout, listOfCategarys[i].CategoryDesc.ToString(), listOfCategarys[i].Category_ID, listOfCategarys[i].RatingID);
            }
        }

        public void VisibleOrInVisibleScreenDesign(List<Ratings> listOfRations, StackLayout categaryStacklayoutRef, string styleIdRef, int mainCategaryId, string defaultRating)
        {
            ...
            OverAllRatingsDesign(listOfRations, ratingsLayout, mainCategaryId, defaultRating);
            ...
            secondPartLayoutDesign(listOfRations, listOfEnterprisePerCategary, visibleOrInVisibleMainStack, "Corporate Factors", mainCategaryId);
        ...
        public void secondPartLayoutDesign(List<Ratings> listOfRations, List<EntBehavior> listOfObjectsPerCategary, StackLayout secondStackRef, string titleLabel, int CategaryId)
        {
            ...
            for (int i = 0; i < listOfObjectsPerCategary.Count; i++)
            {
                ...
                SubRatingsDesignLayout(listOfRations, ratingsLayout, listOfObjectsPerCategary[i].Behavior_ID, CategaryId, listOfObjectsPerCategary[i].BehaviorType_ID, listOfObjectsPerCategary[i].Rating_ID.ToString());
        ...
        public void ThirdPartLayoutDesign(List<OrgBehavior> listOfObjectsPerCategary, StackLayout secondStackRef, string titleLabel, int CategaryId)
        {
            ...
            for (int i = 0; i < listOfObjectsPerCategary.Count; i++)
            {
                ...
                SubRatingsDesignLayout(listOfRatings, ratingsLayout, listOfObjectsPerCategary[i].Behavior_ID, CategaryId, listOfObjectsPerCategary[i].BehaviorType_ID, listOfObjectsPerCategary[i].Rating_ID.ToString() );
        ...
        public void takePhotoOrAlbumStack(StackLayout visibleOrInvisible, int CategaryId)
        {
            ...
            saveButtoninCategary.IsVisible = false;
        ...
        public void OverAllRatingsDesign(List<Ratings> listOfRatings, StackLayout layout, int mainId, string defaultRating)
        {
            ...
            for (int i = 0; i < listOfRatings.Count; i++)
            {
                ...
                if (!string.IsNullOrEmpty(defaultRating))
                {
                    if (i == 0)
                    {
                        if (i + 1 == Int32.Parse(defaultRating))
                        {
                            exceptionalBtn.Image = "factorExceptionalActive.png";
                        }
                        else
                        {
                            exceptionalBtn.Image = "factorExceptionalInactive.png";
                    ...
                    if (i == 1)
                    {
                        if (i + 1 == Int32.Parse(defaultRating))
                        {
                            exceptionalBtn.Image = "factorEffectiveActive.png";
                        }
                        else
                        {
                            exceptionalBtn.Image = "factorEffectiveInactive.png";
        ...
        public void SubRatingsDesignLayout(List<Ratings> listOfRatings, StackLayout layout, int BehaviourId, int CategaryId, int BehaviourTypeId, string defaultRating)
        {
        ...
        public Task<int> ReadDataFromJson()
        {
            ...
            Stream stream = assembly.GetManifestResourceStream("SampleDB.EditCommunications.json");

            using (var reader = new System.IO.StreamReader(stream))
            {

                var json = reader.ReadToEnd();
                EditCommunicationResponce data = JsonConvert.DeserializeObject<EditCommunicationResponce>(json);

                arrayobj = data.Categories;


                for (int k = 0; k < arrayobj.Count(); k++)
                {
                    JSSEMasterCategory mctbl = new JSSEMasterCategory();
                    mctbl.Category_ID = arrayobj[k].Category_ID;
                    mctbl.Category = arrayobj[k].CategoryName;
                    ...
                    List<EntBehavior> eorgdata = arrayobj[k].EntBehaviors;
                    for (int l = 0; l < eorgdata.Count(); l++)
                    {
                    ...
                    for (int i = 0; i < arrayobj[k].OrgBehaviors.Count; i++)
                    {
                        for (int j = 0; j < arrayobj[k].OrgBehaviors.Count(); j++)
                        {
                            JSSEMasterBehavior mbhtbl = new JSSEMasterBehavior();

                            //mbhtbl.Behavior_ID = arrayobj[k].OrgBehaviors[i][j].ca;
